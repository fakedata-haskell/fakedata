...
resolveVehicleText :: (MonadIO m, MonadThrow m) => FakerSettings -> Text -> m Text
...
resolveVehicleField :: (MonadThrow m, MonadIO m) => FakerSettings -> Text -> m Text

resolveVehicleField settings undefined =
  randomUnresolvedVec settings license_plateProvider resolveLicense_plateText
